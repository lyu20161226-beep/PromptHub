...
export const packDeepDives: Record<string, PackDeepDive> = {
  "xiaohongshu-growth": {
    ...
    mistakes: ["只写产品功能，不写用户场景。", "标题太像广告，缺少情绪钩子。", "正文没有转化结构。", "标签过泛，无法承接搜索流量。"]
  },
  "cross-border-commerce": {
    slug: "cross-border-commerce",
    goal: "帮助跨境卖家把一个商品快速拆成 Listing、主图、广告和售后内容。",
    promise: "从关键词到 Review 回复，覆盖商品页每天都会用到的核心内容资产。",
    fullWorkflow: ["关键词分析", "标题生成", "Bullet Points", "A+ Content", "广告文案", "Review 回复"],
    prompts: ["关键词 Prompt", "标题 Prompt", "五点描述 Prompt", "A+ 内容 Prompt", "广告文案 Prompt", "Review 回复 Prompt"],
    beforeAfter: {
      ...
      after: [
        ...
        "Bullet：按通勤、会议、运动三个场景组织卖点，而不是堆功能词。",
        "A+：输出模块标题、图片说明、对比表和 FAQ。",
        "Review：区分物流、质量、使用误解三类回复模板。"
      ]
    },
    demoSteps: ["填写商品参数", "生成关键词簇", "输出 Listing 与五点描述", "生成主图需求", "生成广告与 Review 回复"],
    testimonial: {
      quote: "最有价值的是把 Listing、主图和广告文案串起来了，不用在几个文档里来回复制。",
      role: "Amazon 3C 配件卖家"
    },
    roi: {
      traditional: "1 天完成商品页初稿，反复切换关键词、文案和视觉需求。",
      withPack: "90 分钟完成第一版 Listing、主图需求和广告文案。",
    ...
    mistakes: ["标题堆关键词导致可读性差。", "Bullet 只写参数，不写场景利益。", "A+ 内容缺少视觉说明。", "Review 回复过度承诺，带来合规风险。"]
  },
  "indie-product-research": {
    ...
    prompts: ["Idea 筛选 Prompt", "市场验证 Prompt", "竞品分析 Prompt", "Landing Page Prompt", "定价 Prompt", "冷启动营销 Prompt"],
    beforeAfter: {
      input: "想法：给自由职业者做一个 AI 合同审查工具；目标：英文市场；预算：低成本验证。",
      before: "这个想法不错，可以做一个网站，然后推广给自由职业者。",
      after: [
        "市场验证：先验证高频痛点是付款条款、责任边界还是版权归属。",
        "竞品分析：拆解 LegalZoom、DocuSign、独立 AI 合同工具的定价和入口。",
        "Landing Page：输出首屏标题、3 个痛点模块、等待名单 CTA。",
        "冷启动：列出 Reddit、Indie Hackers、LinkedIn 的 14 天发布计划。"
      ]
    },
    ...
    outputTemplate: ["用户画像", "竞品矩阵", "差异化机会", "Landing Page 文案", "定价假设", "30 天冷启动计划"],
    mistakes: ["先写代码，后找用户。", "竞品分析只看功能，不看获客和定价。", "Landing Page 没有明确等待名单 CTA。", "定价没有和目标用户预算匹配。"]
  }
};
