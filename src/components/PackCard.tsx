...
import { ArrowRight, BookOpen, BriefcaseBusiness, Check, GraduationCap, Images, SearchCheck, ShoppingBag } from "lucide-react";
  ...
  return (
    <article className="flex h-full flex-col rounded-lg border border-zinc-200 bg-white p-5 shadow-sm transition hover:border-emerald-400 hover:shadow-md sm:p-6">
      <div className="flex items-start justify-between gap-4">
        ...
        <span className="rounded-full bg-emerald-50 px-2.5 py-1 text-xs font-semibold text-emerald-800">可免费预览</span>
      </div>
      ...
      {!compact && (
        <ul className="mt-5 space-y-2 border-t border-zinc-100 pt-5">
          {pack.includes.map((item) => (
            <li className="flex gap-2 text-sm text-zinc-700" key={item}>
      ...
      <div className="mt-auto flex items-end justify-between gap-4 pt-6">
        <div>
          <span className="text-xs text-zinc-400">一次性验证价</span>
          <p className="mt-0.5 text-2xl font-bold text-zinc-950">¥{pack.price}</p>
        </div>
        <Link className="inline-flex min-h-10 items-center justify-center gap-2 rounded-md bg-zinc-950 px-4 text-sm font-semibold text-white hover:bg-emerald-700" href={`/packs/${pack.slug}`}>
          查看内容 <ArrowRight className="h-4 w-4" aria-hidden="true" />
        </Link>
      </div>
    </article>
  );
