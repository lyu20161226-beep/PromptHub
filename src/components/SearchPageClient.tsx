          ...
          <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {results.map((prompt) => (
              <article
                ...
                key={prompt.id}
              >
                <Link aria-label={`查看${prompt.title}详情`} className="absolute inset-0 rounded-lg" href={prompt.workflow ? `/workflows/${prompt.slug}` : `/prompts/${prompt.slug}`} onClick={() => prompt.workflow && void recordValidationEvent("workflow_click", { workflowId: prompt.id, source: "/search" })} />
